...
<div style="width:300px;height:100vh;background:#ddd;position:fixed;top:0;left:0" id="source-container" data-role="drag-drop-container">
    ...
    <div style="background-image:url(img/radio.png)" class="list_style background-limit" id="drag-source5" draggable="true" value='radio'></div>
    <div style="background-image:url(img/checkbox.png)" class="list_style background-limit" id="drag-source6" draggable="true" value='checkbox'></div>
    <hr>
    ...
    if(node_name!=undefined){
        ...
        if(allData[node_name.nodeValue]["font"]){
            ...
            if(allData[node_name.nodeValue]["font-size"])
                td_node.appendChild(create_form("字體大小","input","font-size",node_style,false));
            if(allData[node_name.nodeValue]["color"])
                td_node.appendChild(create_form("字體顏色","select","color",node_style,true));
            if(allData[node_name.nodeValue]["font-family"])
                td_node.appendChild(create_form("字型","select","font-family",node_style,true));
            if(allData[node_name.nodeValue]["font-weight"])
                td_node.appendChild(create_form("字體粗細","select","font-weight",node_style,true));
            if(allData[node_name.nodeValue]["text-align"])
                td_node.appendChild(create_form("對齊方式","select","text-align",node_style,true));
        }
